[**Question Link**](https://www.hackerrank.com/challenges/array-splitting/problem)
...
int rec(int l,int r){
    ...
    while(hi-lo>1){
        mid=(hi+lo)/2;
        if(s[mid]-s[l-1]<=req) lo=mid;
        else hi=mid;
    }
    if(s[lo]-s[l-1]!=req) return 0;
    return 1+max(rec(l,lo),rec(lo+1,r));
}
int main(){
    int t;
    cin >> t;
